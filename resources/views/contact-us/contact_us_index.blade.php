                    ...
                    <div class="card card-docs mb-2">
                        <div class="p-10">
                            <h1 class="anchor fw-bolder mb-5" id="zero-configuration">
                                <a href="javascript:;"></a>Queries Listing
                            </h1>
                            <div class="d-flex align-items-center rounded py-5 px-4 bg-light-primary">
                                ...
                                <div class="text-gray-700 fw-bold fs-6 lh-lg">Here is the list of all of the Queries that we have.</div>
                            </div>
                        </div>
                            ...
                            <div class="py-5">
                                <table class="kt_datatable_example_1 table table-row-bordered gy-5">
                                    ...
                                    <tbody class="fw-bold text-gray-600">
                                        @for ($i = 0; $i < count($contacts); $i++) @php $a=$i; $a++; @endphp

                                        <tr>
                                            <td><a href="#" onclick="showContact('{{$contacts[$i]->id}}')" class="fw-bolder text-gray-800 text-hover-primary mb-1">{{$a}}</a></td>
                                            <td>
                                                <a href="#" onclick="showContact('{{$contacts[$i]->id}}')" class="text-gray-800 text-hover-primary mb-1">{{ucwords($contacts[$i]->name)}}</a>
                                            </td>
                                            <td>{{$contacts[$i]->email}}</td>
                                            <td>{{$contacts[$i]->city}}</td>
                                            <td class="w-50">{{substr($contacts[$i]->message, 0, 100).'...'}}</td>
                                            <td>{{date("Y-m-d",strtotime($contacts[$i]->created_at))}}</td>
                                            <td>
                                                <button onclick="editContact('{{$contacts[$i]->id}}')" class="btn btn-icon btn-sm btn-color-gray-400 btn-active-icon-primary me-2"  data-bs-original-title="Edit Query">
                                                ...
                                                <form  style="display: inline-block" method="POST" action="{{ route('contact_us.destroy', $contacts[$i]->id) }}">
                                                    @method('DELETE')
                                                    @csrf
                                                <button class="btn btn-icon btn-sm btn-color-gray-400 btn-active-icon-danger me-2" data-bs-toggle="tooltip" data-bs-original-title="Delete Query">
                                                    <span class="svg-icon svg-icon-2">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
...
function showContact(id) {
        url = "{{ route('contact_us.show', ':id') }}";
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            success: function(result) {
                $('#myModalLgHeading').html('View Query');
    ...
    function editContact(id) {
        url = "{{ route('contact_us.edit', ':id') }}";
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            success: function(result) {
                $('#myModalLgHeading').html('Edit Query');
                $('#modalBodyLarge').html(result);
                $('#myModalLg').modal('show');
            }
        });
