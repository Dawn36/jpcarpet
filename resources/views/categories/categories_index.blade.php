                    ...
                    <div class="card card-docs mb-2">
                        <div class="p-10">
                            <h1 class="anchor fw-bolder mb-5" id="zero-configuration">
                                <a href="javascript:;"></a>Categories Listing
                            </h1>
                            <div class="d-flex align-items-center rounded py-5 px-4 bg-light-primary">
                                ...
                                <div class="text-gray-700 fw-bold fs-6 lh-lg">Here is the list of all of the Categories that we have.</div>
                            </div>
                        </div>
                        <div class="card-header border-0">
                            ...
                            <div class="card-toolbar">
                                <div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
                                    <button type="button" class="btn btn-primary me-2" onclick="addCategories()">
                            ...
                            <div class="py-5">
                                <table class="kt_datatable_example_1 table table-row-bordered gy-5">
                                    ...
                                    <tbody class="fw-bold text-gray-600">
                                        @for ($i = 0; $i < count($categories); $i++) @php $a=$i; $a++; @endphp
                                        <tr>
                                            <td>{{$a}}</td>
                                            <td>{{ucwords($categories[$i]->name)}}</td>
                                            <td>{{$categories[$i]->blogs_count}}</td>
                                            <td>{{Date("Y-m-d",strtotime($categories[$i]->created_at))}}</td>
                                            <td>
                                                <button class="btn btn-icon btn-sm btn-color-gray-400 btn-active-icon-primary me-2"  onclick="editCategories('{{$categories[$i]->id}}')" data-bs-original-title="Edit Category">
                                                    <span class="svg-icon svg-icon-2">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                                ...
                                                <form  style="display: inline-block" method="POST" action="{{ route('categories.destroy', $categories[$i]->id) }}">
                                                    @method('DELETE')
                                                    @csrf
                                                <button class="btn btn-icon btn-sm btn-color-gray-400 btn-active-icon-danger me-2" data-bs-toggle="tooltip" data-bs-original-title="Delete Category">
                                                    <span class="svg-icon svg-icon-2">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
    ...
    function editCategories(id) {
        ...
        $.ajax({
            ...
            success: function(result) {
                $('#myModalLgHeading').html('Edit Categories');
